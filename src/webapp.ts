...
export const APP_HTML = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light dark"><title>AnonInbox</title>
...
button{border:0;border-radius:10px;padding:12px 14px;font-size:15px;background:var(--tg-theme-button-color,#2ea6ff);color:var(--tg-theme-button-text-color,#fff);width:100%;margin-bottom:10px}.link{font-size:13px;word-break:break-all;color:var(--tg-theme-hint-color,#777);margin-bottom:10px}.empty{color:var(--tg-theme-hint-color,#777)}#pro button{width:100%}</style></head><body>
<h1 data-i18n="app_title"></h1><div class="link" id="link"></div><button id="share" data-i18n="app_shareLink"></button><div id="stats" class="empty"></div><div id="list" class="empty" data-i18n="app_loading"></div>
...
function renderShare(){const el=document.getElementById("shareRow");if(!el)return;let ok=false;try{ok=typeof tg.shareMessage==="function"&&tg.isVersionAtLeast("8.0")}catch(e){}
 ...
 if(ok2){const s=sBtn(T('app_shareStory'),"8px");s.onclick=()=>{try{api("/api/share-story",{}).catch(()=>{});tg.shareToStory("https://tg.zovo.one/img/banner-anon.png",{text:T('app_storyText')+"\\n\\n${STORY_LINK}",widget_link:{url:"${STORY_LINK}",name:"AnonInbox"}})}catch(e){}};el.appendChild(s)}}
function msgRow(m){const row=document.createElement('div');row.className='m';
 row.appendChild(document.createTextNode(m.body));
 if(m.answer){const a=document.createElement('div');a.className='a';a.textContent='💬 '+m.answer;row.appendChild(a)}
 const s=document.createElement('small');
 s.textContent=new Date(m.created*1000).toLocaleString()+(m.replied?' · replied':'')+(m.answer?' · answered publicly':'');
 row.appendChild(s);return row}
async function load(){let d;try{d=await api('/api/inbox',{})}catch(e){d=null}
 const el=document.getElementById('list');
 if(!d||d.error){el.className='empty';el.textContent=errText(d);return}
 renderPro(d);
 document.getElementById('link').textContent=d.link;
 document.getElementById('share').onclick=()=>tg.openTelegramLink('https://t.me/share/url?url='+encodeURIComponent(d.link)+'&text='+encodeURIComponent(T('app_pitch')));
 document.getElementById('stats').textContent=d.total+' messages · '+d.opens+' link opens'+(d.pro?' · Pro':'');
 if(!d.messages.length){el.className='empty';el.textContent=T('app_empty');return}
 el.className='';el.innerHTML='';for(const m of d.messages)el.appendChild(msgRow(m))}
