...
const APP_KEYS = ["app_title","app_loading","app_moreApps","app_empty","app_shareChat","app_shareStory","app_shareFail","app_shareLink","app_pitch","app_storyText","app_errNoInit","app_errExpired","app_errBadSig","app_unlockPro","app_proOneTime","app_proMonthly","app_payDone","app_payCancelled","app_payFailed"];
/** app_* keys rendered as a button: Telegram truncates a long label on narrow screens. */
const BUTTON_KEYS = ["app_shareChat","app_shareStory","app_shareLink","app_unlockPro","app_proOneTime","app_proMonthly"];
...
test("pro-link: a real user gets the minted link for the plan they asked for", async () => {
  ...
  // anon has no monthly plan (allowMonthly: false): "monthly" degrades to one-time
  // rather than minting a subscription anon can never fulfill.
